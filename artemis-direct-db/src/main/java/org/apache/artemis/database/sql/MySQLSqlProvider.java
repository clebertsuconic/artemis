...
public class MySQLSqlProvider extends SQLProvider {
   ...
   public String createQueue(String tableName) {
      return String.format("CREATE TABLE IF NOT EXISTS %s(QUEUE_ID BIGINT NOT NULL, ADDRESS_ID BIGINT NOT NULL, QUEUE_NAME VARCHAR(255), IS_MULTICAST CHAR, IS_ANYCAST CHAR, FILTER_STRING LONGBLOB, QUEUE_CONFIG LONGBLOB, PRIMARY KEY (QUEUE_ID))", tableName);
   }

   @Override
   public String createPage(String tableName) {
      return String.format("CREATE TABLE IF NOT EXISTS %s(ADDRESS_ID BIGINT NOT NULL, PAGE_ID BIGINT NOT NULL, PAGE_NR BIGINT NOT NULL, MESSAGE_ID BIGINT NOT NULL, TX_ID BIGINT, MESSAGE_RECORD LONGBLOB, PRIMARY KEY (ADDRESS_ID, PAGE_ID, PAGE_NR))", tableName);
   }

   @Override
   public String createPageReferences(String tableName) {
      return String.format("CREATE TABLE IF NOT EXISTS %s(ADDRESS_ID BIGINT NOT NULL, PAGE_ID BIGINT NOT NULL, PAGE_NR BIGINT NOT NULL, QUEUE_ID BIGINT NOT NULL, PRIMARY KEY (ADDRESS_ID, PAGE_ID, PAGE_NR, QUEUE_ID))", tableName);
   }

   @Override
   public String createGenericData(String tableName) {
      return String.format("CREATE TABLE IF NOT EXISTS %s(ID BIGINT NOT NULL, RECORD_TYPE SMALLINT NOT NULL, TX_ID BIGINT, DATA_RECORD LONGBLOB, PRIMARY KEY (ID))", tableName);
   }
}
